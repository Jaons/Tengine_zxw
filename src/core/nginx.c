...
static void ngx_test_fun(char *test_case);
...
main(int argc, char *const *argv)
{
    ...
    // 初始化ngx_strerror，因为strerror为非异步信号安全的，因此需要封装一个
        ...
        while (*p) {
            ...
            case 'T':
                if (*p) {
                    ...
                    ngx_log_stderr(0, "option \"-T\" requires parameter");
                    return NGX_ERROR;
                }
...
static void ngx_test_fun(char *test_case)
{
    ...
    pool = ngx_create_pool(8 * NGX_CYCLE_POOL_SIZE,  log);
    
    ngx_array_init(&array, pool, 32, sizeof(ngx_hash_key_t));

    for (i = 0; i < 10; i ++) {
        hk = ngx_array_push(&array);
        hk->key.len = 5;
        sprintf((char *)hk->key.data, "key%02d", i);
        hk->key_hash = ngx_hash_key_lc(hk->key.data, hk->key.len);
        hk->value = NULL;
    }
    

    if (strcmp(test_case, "ngx_hash") == 0)
    {

        
        ihash.hash = NULL;
        ihash.key = ngx_hash_key_lc;
        ihash.max_size = 8;
        ihash.bucket_size = 4096;
        ihash.name = "test_hash";
        ihash.pool = pool;                                                                                                                 
        ihash.temp_pool = NULL;

        ngx_hash_init(&ihash, array.elts, array.nelts); 
    }
