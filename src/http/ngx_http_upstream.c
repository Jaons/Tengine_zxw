...
ngx_http_upstream_init(ngx_http_request_t *r)
{
    ...
    // add by zhouxw 20170713 begin 
#if (NGX_HTTP_AUDIT)
    ngx_chain_t                 *cl;
    size_t                      len;
#endif
    // add by zhouxw 20170713 end 

    // add by zhouxw 20170713 begin
#if (NGX_HTTP_AUDIT)
    // 请求体
    if (r->headers_in.content_length_n) {
        r->http_audit_info->request_body.data = ngx_pcalloc(r->pool, r->headers_in.content_length_n);
        //r->http_audit_info->request_body.len = r->headers_in.content_length_n;
        if (r->http_audit_info->request_body.data == NULL) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                          "http audit alloc request_body.data error");
            ngx_http_finalize_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
            return NGX_HTTP_INTERNAL_SERVER_ERROR;
        }
        // 此处不可用ngx_cpystrn处理，此函数处理以'\0'结束的字符串
        cl = r->request_body->bufs;
        if (cl->next == NULL) {
            len = cl->buf->last - cl->buf->pos;
            if (len == 0) {
                r->http_audit_info->request_body.len = 0;
            }
            ngx_memcpy(r->http_audit_info->request_body.data, (char *)cl->buf->pos, len);   
            r->http_audit_info->request_body.len = len;
        } else {
            len = 0;
            ...
            for (cl = r->request_body->bufs; cl; cl = cl->next) {
                ngx_memcpy(r->http_audit_info->request_body.data + len, (char *)cl->buf->pos, cl->buf->last - cl->buf->pos);
                len += cl->buf->last - cl->buf->pos;
            }
        }
    }
...
ngx_http_upstream_connect(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    // 此处做审计，审计失败则直接返回
    // 20170630 添加
    // 因为上面注册了连接的读写事件，因此简单返回的话，会导致事件处理时回调函数为空指针，此处需要对注册的事件做处理
#if (NGX_HTTP_AUDIT)
    // 取客户端地址信息
    ...
    default: 
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "http audit unsupported protocol family");
        ngx_http_upstream_finalize_request(r, u,
                                           NGX_HTTP_INTERNAL_SERVER_ERROR);
        return ;
    }
    
    r->http_audit_info->local_addr.data = ngx_pcalloc(r->pool, len + 1);
    if (r->http_audit_info->local_addr.data == NULL) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "http audit alloc local_addr.data error");
        ngx_http_upstream_finalize_request(r, u,
                                           NGX_HTTP_INTERNAL_SERVER_ERROR);
        return ;
    }

    len = ngx_sock_ntop(r->connection->sockaddr, r->connection->socklen, r->http_audit_info->local_addr.data, len, 1);
    if (len == 0) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "http audit ngx_sock_ntop error");
        ngx_http_upstream_finalize_request(r, u,
                                           NGX_HTTP_INTERNAL_SERVER_ERROR);
        return ;
    }

    *(r->http_audit_info->local_addr.data + len) = '\0';
    if ((ptr = (u_char *)strchr((const char*)r->http_audit_info->local_addr.data, ':')) == NULL) {
        // 客户端发出的必须指定端口
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                        "http audit client IP info without port");
        ngx_http_upstream_finalize_request(r, u,
                                           NGX_HTTP_INTERNAL_SERVER_ERROR);
        return ;
    }
    ...
    *ptr = '\0';
    r->http_audit_info->local_port = ngx_atoi(ptr + 1, len - (ptr - r->http_audit_info->local_addr.data) - 1);

    // 上游服务器
    r->http_audit_info->remote_addr.data = ngx_pcalloc(r->pool, r->upstream->peer.name->len + 1);
    if (r->http_audit_info->remote_addr.data == NULL) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "http audit alloc remote_addr.data error");
        ngx_http_upstream_finalize_request(r, u,
                                           NGX_HTTP_INTERNAL_SERVER_ERROR);
        return ;
    }

    ngx_cpystrn(r->http_audit_info->remote_addr.data, r->upstream->peer.name->data, r->upstream->peer.name->len + 1);

    if ((ptr = (u_char *)strchr((const char*)r->http_audit_info->remote_addr.data, ':')) == NULL) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                        "http audit upstream svr_IP info without port");
        ngx_http_upstream_finalize_request(r, u,
                                           NGX_HTTP_INTERNAL_SERVER_ERROR);
        return ;
    }
    *ptr = '\0';
    r->http_audit_info->remote_port = ngx_atoi(ptr + 1, r->upstream->peer.name->len - (ptr - r->http_audit_info->remote_addr.data) - 1);

    /*
    // 请求数据
    r->http_audit_info->request_content.data = ngx_pcalloc(r->pool, r->request_length + 1);
    r->http_audit_info->request_content.len = r->request_length;
    if (r->http_audit_info->request_content.data == NULL) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "http audit alloc request_content.data error");
        ngx_http_upstream_finalize_request(r, u,
                                           NGX_HTTP_INTERNAL_SERVER_ERROR);
        return ;
    }
    // 此处不可用ngx_cpystrn处理，此函数处理以'\0'结束的字符串
    //ngx_cpystrn(request_content.data, r->request_start, r->request_length + 1);
    ngx_memcpy(r->http_audit_info->request_content.data, r->request_start, r->request_length);
    *(r->http_audit_info->request_content.data + r->request_length + 1) = '\0';
    while ((ptr = (u_char*)strchr((const char *)r->http_audit_info->request_content.data, '\0')) != NULL && (ptr - r->http_audit_info->request_content.data < r->request_length)) {
        switch (*(ptr + 1)) {
            case    32:
                *ptr = ':';
                break;
            case    10:
                *ptr = '\r';
                break;
            default :    
                ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "http audit invalid request_content[%c][%V]", *(ptr + 1), &r->http_audit_info->request_content);
                ngx_http_upstream_finalize_request(r, u,
                                           NGX_HTTP_INTERNAL_SERVER_ERROR);
                return ;
        }
    ...
    // libhtp
    r->http_audit_info->cfg = htp_config_create();
    htp_config_set_server_personality(r->http_audit_info->cfg, HTP_SERVER_GENERIC);
    htp_config_register_urlencoded_parser(r->http_audit_info->cfg);
    htp_config_register_multipart_parser(r->http_audit_info->cfg);

    gettimeofday(&r->http_audit_info->tv_start, NULL);
    r->http_audit_info->connp = htp_connp_create(r->http_audit_info->cfg);
    if (r->http_audit_info->connp == NULL) {
        //fprintf(stderr, "Failed to create connection parser\n");
        //exit(1);
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "http audit Failed to create connection parser");
        ngx_http_upstream_finalize_request(r, u,
                                           NGX_HTTP_INTERNAL_SERVER_ERROR);
        return ;
    }
    htp_connp_set_user_data(r->http_audit_info->connp, (void *) 0x02);

    htp_connp_open(r->http_audit_info->connp, (const char *) r->http_audit_info->remote_addr.data, r->http_audit_info->remote_port, (const char *) r->http_audit_info->local_addr.data, r->http_audit_info->local_port, &r->http_audit_info->tv_start);

    htp_connp_req_data(r->http_audit_info->connp, &r->http_audit_info->tv_start, r->http_audit_info->request_content.data, r->http_audit_info->request_content.len);
    */
    ...
    tx = htp_list_get(connp->conn->transactions, 0);
...
ngx_http_upstream_process_header(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    // add by zhouxw 20170706 begin 
#if (NGX_HTTP_AUDIT)
    ngx_int_t           lenOfResHead;
    u_char              *ptr;
#endif
    // add by zhouxw 20170706 end 
...
#if (NGX_HTTP_AUDIT)
    // 响应头
    ptr = (u_char *)strstr ((char*)u->buffer.start, "\r\n\r\n");
    lenOfResHead = ptr - u->buffer.start + 4;
    r->http_audit_info->response_header.data = ngx_pcalloc(r->pool, lenOfResHead);
    r->http_audit_info->response_header.len = lenOfResHead;
    if (r->http_audit_info->response_header.data == NULL) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "http audit alloc response_header.data error");
        ngx_http_upstream_finalize_request(r, u,
                                           NGX_HTTP_INTERNAL_SERVER_ERROR);
        return ;
    }
    // 此处不可用ngx_cpystrn处理，此函数处理以'\0'结束的字符串
    //ngx_cpystrn(request_content.data, r->request_start, r->request_length + 1);
    ngx_memcpy(r->http_audit_info->response_header.data, u->buffer.start, lenOfResHead);
#endif
...
ngx_http_upstream_send_response(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    // add by zhouxw 20170718 begin
    // 响应体 , 预分配内存
#if (NGX_HTTP_AUDIT)
    if (r->upstream->headers_in->content_length_n) {
        r->http_audit_info->response_body.data = ngx_pcalloc(r->pool, r->upstream->headers_in.content_length_n);
        r->http_audit_info->response_body.len = 0;
    }
#endif
    // add by zhouxw 20170718 end
