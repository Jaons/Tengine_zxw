...
ngx_http_wait_request_handler(ngx_event_t *rev)
{
    ...
    /*
    r->http_audit_info = ngx_pcalloc(r->pool, sizeof(ngx_http_audit_t));
    if (r->http_audit_info == NULL) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "http audit alloc http_audit_info error");
        ngx_http_close_connection(c);
        return ;
    }
    */
    ptr = (u_char*)strstr((char*)b->pos, "\r\n\r\n");
    len = ptr - b->pos + 4;
    r->http_audit_info->request_header.data = ngx_pcalloc(r->pool, len);
    if (r->http_audit_info->request_header.data == NULL) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "http audit alloc request_header.data error");
        ngx_http_close_connection(c);
        return ;
    }
    // 此处不可用ngx_cpystrn处理，此函数处理以'\0'结束的字符串
    r->http_audit_info->request_header.len = len;
    ngx_memcpy(r->http_audit_info->request_header.data, b->pos, len);
#endif
    // add by zhouxw 20170713 end
...
ngx_http_process_request_headers(ngx_event_t *rev)
{
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_HTTP_PARSE_HEADER_DONE) {
...
#if (NGX_HTTP_AUDIT)
            // 请求头
            r->http_audit_info->request_header.data = ngx_pcalloc(r->pool, r->request_length);
            r->http_audit_info->request_header.len = r->request_length;
            if (r->http_audit_info->request_header.data == NULL) {
                ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                              "http audit alloc request_header.data error");
                return ;
            ...
            ngx_memcpy(r->http_audit_info->request_header.data, b->pos, n);
#endif
            // add by zhouxw 20170713 end
            */
