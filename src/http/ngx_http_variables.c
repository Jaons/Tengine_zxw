...
static ngx_int_t ngx_http_variable_full_request(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
// add by zhouxw 20170725 begin
static ngx_int_t ngx_http_variable_upstream_info(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
//static ngx_int_t ngx_http_variable_remote_mac(ngx_http_request_t *r,
//    ngx_http_variable_value_t *v, uintptr_t data);
// add by zhouxw 20170725 end
...
static ngx_http_variable_t  ngx_http_core_variables[] = {
    ...
    // add by zhouxw 20170725 begin
    { ngx_string("upstream_host"), NULL, ngx_http_variable_upstream_info,
      0, NGX_HTTP_VAR_NOCACHEABLE, 0 },
//    { ngx_string("upstream_mac"), NULL, ngx_http_variable_upstream_info,
//      1, NGX_HTTP_VAR_NOCACHEABLE, 0 },
//    { ngx_string("remote_mac"), NULL, ngx_http_variable_remote_mac,
//      0, NGX_HTTP_VAR_NOCACHEABLE, 0 },
    // add by zhouxw 20170725 end
...
// add by zhouxw 20170725 begin
static ngx_int_t
ngx_http_variable_upstream_info(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
    size_t                     size = 0;
    ngx_str_t                  addr, port;
...
#if (NGX_HAVE_INET6)
    case AF_INET6:
        addr.data = ngx_pnalloc(r->pool, NGX_INET6_ADDRSTRLEN);
        if (addr.data == NULL) {
            goto failed;
        }
        ...
        s_len = sizeof(sin6);
        getsockopt(r->connection->fd, SOL_IP, SO_ORIGINAL_DST, &sin6, &s_len);
        addr.len = ngx_inet_ntop(AF_INET6, &(sin6.sin6_addr.s6_addr), addr.data, NGX_INET6_ADDRSTRLEN);
        p = ntohs(sin6.sin6_port);
        break;
#endif

    default:
        addr.data = ngx_pnalloc(r->pool, NGX_INET_ADDRSTRLEN);
        if (addr.data == NULL) {
            goto failed;
        }
        ...
        s_len = sizeof(sin);
        getsockopt(r->connection->fd, SOL_IP, SO_ORIGINAL_DST, &sin, &s_len);
        addr.len = ngx_inet_ntop(AF_INET, &(sin.sin_addr.s_addr), addr.data, NGX_INET_ADDRSTRLEN);
        p = ntohs(sin.sin_port);
        break;
    }
    ...
    switch(data) {
        case 0:
            size = addr.len + port.len;

            v->data = ngx_pnalloc(r->pool, NGX_INET6_ADDRSTRLEN + sizeof("[]:65535") - 1);
            if (v->data == NULL) {
                goto failed;
            }
            ...
            last = ngx_cpymem(last, addr.data, addr.len);
...
static ngx_int_t
ngx_http_variable_upstream_host(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
    size_t                     size;
    ngx_str_t                  addr, port;
